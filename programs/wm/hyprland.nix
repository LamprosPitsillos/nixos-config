...
let
  ...
  monitors = (
    map (
      ...
      "${m.name},${
        if m.enabled then
          "${toString m.width}x${toString m.height}@${toString m.refreshRate},${toString m.x}x${toString m.y},${toString m.scale}"
        else
          "disable"
      }"
    ) osConfig.custom.hostProps.monitors
  );
in
{
  ...
  wayland.windowManager.hyprland = {
    ...
    extraConfig =
      ...
      ''
                ...
                bind = SHIFT,ALT ,exec, hyprctl switchxkblayout kanata next
