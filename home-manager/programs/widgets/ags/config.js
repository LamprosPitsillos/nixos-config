...
function Workspaces() {
    const WORKSPACES_LEN = 5
    const activeId = hyprland.active.workspace.bind("id")
    return Widget.Box({
        class_name: "workspaces",
        children: Array.from({ length: WORKSPACES_LEN }, (_, i) => ++i).map(i => Widget.Button({
...
function Calendar(monitor = 0) {
    return Widget.Window({
        name: `calendar-${monitor}`, // name has to be unique
        visible: false,
        // class_name: "bar",
        monitor,
        anchor: ["top", "right"],
...
function Clock() {
    return Widget.Button({
        ...
        onClicked: () => App.ToggleWindow(`calendar-${getActiveMonitorId()}`),
    })
